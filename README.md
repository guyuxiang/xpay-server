...
| `PAY_TO_ADDRESS` | 是 | `0x...` | 收取 USDC 的地址 |
| `GAS_PRIVATE_KEY` | 生产必填 | `0x...` | 代付 gas 的 EVM 私钥；`DRY_RUN=true` 时可不填 |
| `CHAIN_ID` | 否 | `84532` | 链 ID，`84532` 为 Base Sepolia，`8453` 为 Base Mainnet |
| `USDC_ADDRESS` | 否 | `0x...` | USDC 合约地址；支持链会自动使用默认值 |
| `RPC_URL` | 否 | `https://...` | EVM RPC 地址；支持链会自动使用默认 RPC |
